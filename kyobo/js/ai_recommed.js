const REST_API_KEY_airecommend = "071d884bcf93ef15ab325924977ca231";

async function ai_recommendBooks() {
  const query = "한국 도서";
  ...
  try {
    const response = await fetch(url, {
      headers: {
        Authorization: `KakaoAK ${REST_API_KEY_airecommend}`,
      },
    });
    ...
    const list = document.getElementById("ai_pick-list");
        ...
        (book) => `
          <div style="padding:10px; margin:10px; width:180px;">
            <a href="#">
              ...
              <h3 style="font-size:14px; margin:10px 0; text-align: center;">${
                book.title
              }</h3>
            </a>
          </div>
